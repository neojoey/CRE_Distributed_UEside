...
public: 
  ...
  typedef void (* StateTracedCallback)
	...
	//by Joey
	//for recording a connected time
	void SetConnectedTime (Time time);

	Time GetConnectedTime ();
...
public:
  static TypeId GetTypeId (void);
	...
	// by Joey
	void UpdateOutageCount (uint32_t outageCount);
...
private:
  ...
  void DoRecvUeData (EpcX2SapUser::UeDataParams params);
	//by Joey
	void DoRecvOutageCount (EpcX2SapUser::OutageCountParams params);
	void DoRecvGlobalOutageCount (EpcX2SapUser::OutageCountParams params);
...
#if 1
	// for Dist. bias algorithm by Joey 2016.05.20 -> 07.14
	void DoUpdateOutageCount (uint32_t outageCount);
...
#if 1
	// by Joey - getting a value of local outage count
	uint32_t GetLocalOutageCount ();

	// by Joey - getting a value of global outage count
	// called through CphySapUser by PHY layer
...
#if 1
	// by Joey... store the value of outage count
	uint32_t m_localOutageCount;
	uint32_t m_globalOutageCount;

	std::map <uint16_t, uint32_t> m_recvOutageCount; 	// < cell id , outage count > 
#endif
  /**
