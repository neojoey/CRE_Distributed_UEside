...
//Bias Functions of BiasHelper class
//by joey 2016.04.19
#if 1
BiasHelper::BiasHelper ()
{
}
...
void
BiasHelper::SetBias (double a, double b)
{
	rsrpBias = a;
	rsrqBias = b;
}
  ...
  if (m_rsrpSinrSampleCounter==m_rsrpSinrSamplePeriod)
    {
      ...
      NS_LOG_INFO (this << " cellId " << m_cellId << " rnti " << m_rnti << " RSRP " << rsrp << " SINR " << avSinr);
			...
			// for test by Joey
			//std::cout << Simulator::Now() << " at ReportCQI " << " cellId " << m_cellId << " rnti " << m_rnti << " RSRP " << rsrp << " SINR " << avSinr << std::endl;
  ...
  for (it = m_ueMeasurementsMap.begin (); it != m_ueMeasurementsMap.end (); it++)
    {
      ...
      LteUeCphySapUser::UeMeasurementsElement newEl;
...
#if 1
			//for Bias by joey 2016.04.19
			double rpB = 0;
			double rqB = 0;
			uint16_t cId = (*it).first;

			// CRE implementation, using CID to identify pico cell to add offset value
			std::map <uint16_t, double>::iterator itBias = m_bias->biasList.find (cId);
			if ( itBias != m_bias->biasList.end () )
			{
				rpB = itBias->second;
				//for test
				/*
			...
			/*
			for (uint16_t k=0; k < m_bias->biasList.size (); k++)
			{
				std::cout << "HHHHHHHHHHHHHHHHHHHH" << std::endl;
				if (cId == m_bias->biasList.at (k))
				{
			...
			*/
      newEl.m_rsrp = avg_rsrp + rpB;
      newEl.m_rsrq = avg_rsrq + rqB;
#else
...
//             NS_LOG_DEBUG(this << " RNTI " << m_rnti << " RBG " << i << " DL-DCI allocated PRB " << (i*GetRbgSize()) + k);
          ...
          SetSubChannelsForReception (dlRb);
			...
			// for Abs by joey 2016.04.12
			else if (msg->GetMessageType () == LteControlMessage::ABS)
			{
				Ptr<AbsCtrlMessage> abs = DynamicCast <AbsCtrlMessage> (msg);
				absFrame = abs->GetAbs ();
			}
#if 1
			//for Dist. bias algorithm #1 by Joey 2016.05.03
			else if (msg->GetMessageType () == LteControlMessage::OUTAGE)
			{
...
//by Joey
void
LteUePhy::DoUpdateBias (uint16_t cellId, double bias )
{
	m_bias->SetBias ( bias, 0 );	//rsrp = bias, rsrq = 0 for now
	std::map<uint16_t, double>::iterator it = m_bias->biasList.find (cellId);
	if ( it != m_bias->biasList.end () ) {
		it->second = bias;
	} else {
