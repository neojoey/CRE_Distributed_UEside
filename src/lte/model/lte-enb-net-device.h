...
public:
  ...
  void SetCsgIndication (bool csgIndication);
	...
	//for Abs by joey 2016.04.12
	Ptr <LteEnbNetDevice> GetNetDevicePointer ()
	{
		return this->GetObject <LteEnbNetDevice> (); 
	}
		  
	void AddNodeToVirtualNeighbourList (NodeContainer n)
	{
		for (uint32_t i;i<n.GetN ();i++ )
		{   
			Ptr <Node> n0 = n.Get (i);
			AddNodeToVirtualNeighbourList (n0);
		}   
	}
	
	void AddNodeToVirtualNeighbourList (Ptr <Node> n)
	{
		Ptr <LteEnbNetDevice> netDeviceNeigh = n->GetDevice (0)->GetObject <LteEnbNetDevice> (); 
		neighNetDevices.push_back (netDeviceNeigh);
		std::cout << "CellID: " << GetCellId () << " has added coordination neighbour CellID: " << netDeviceNeigh->GetCellId () << std::endl;
	}
	
	Ptr <FfMacScheduler> GetFfMacSchedule (void) const;
...
private:
  ...
  bool m_csgIndication;
	...
	//for Abs by joey 2016.04.12
	std::list <Ptr <LteEnbNetDevice> > neighNetDevices;
