...
//by joey 2016.04.19
#if 1
class BiasHelper : public Object
{
	public:
		...
		double rsrpBias;
		double rsrqBias;
		std::map<uint16_t, double> biasList; 	// list of CIDs  < cid, rsrp_bias >

		void SetBias (double a, double b);
};
#endif
...
#if 1
	//setting bias ptr to the phy object 
	void SetBiasHelper (Ptr<BiasHelper> BiasHelper)
	{
		m_bias = BiasHelper;
	}
#endif
...
private:
  ...
  void DoSetPa (double pa);
	//by Joey
	void DoUpdateBias (uint16_t cellId, double bias);
  ...
  uint8_t m_subframeNo;
	...
	bool absFrame; 		// notify phy layer that this subframe is blanked during this TTI
