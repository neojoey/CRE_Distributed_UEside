...
/*	
		inline bool operator < (MeasValue const& left, MeasValue const& right) {
			if (left.rsrp < right.rsrp ) { return true; }
			if (left.rsrp > right.rsrp ) { return false; }
			return left.rsrq < right.rsrq; 
		}
	*/	
		struct QState 
		{
			double pMbs;		// quantized P_MBS 
			double pPbs;		// quantized P_PBS_max
			uint16_t picoCellId;	// PBS_CellID_max
		};
/*
		friend QState& operator = (QState &lhs, const QState &rhs) {
			lhs.pMbs = rhs.pMbs;
			lhs.pPbs = rhs.pPbs;
			lhs.picoCellId = rhs.picoCellId;
			return *lhs;
		}
*/
		typedef std::map <double, double> IN_QTABLE;	// < Action(bias), Q-value > value in QTABLE
		
		// < P_MBS, P_PBS_max, PBS_CellID_max,  Action(bias), Q-value > 
		typedef std::map <QState, std::map <double, double> > QTABLE;

		friend bool operator < (const QState &lhs, const QState &rhs) {
			//if (lhs->pMbs < rhs.pMbs) { return true; }
			//if (lhs->pMbs > rhs.pMbs) { return false; }
			//return lhs->pPbs < rhs.pPbs; 
			return ( (lhs.pMbs < rhs.pMbs) && (lhs.pPbs < rhs.pPbs) );
		}

		void InvokeBias (std::map <uint16_t, MeasValues> storedMeasValues, uint32_t outageCount, uint64_t imsi);
		void Quantization (std::map <uint16_t, MeasValues> storedMeasValues );
		void MakeState (std::map <uint16_t, MeasValues> quantizedMeasValues);

		// Add and Update Q-Table
		void UpdateQTable (QState currentQState, uint32_t outageCount);
		void SelectAction (QState currentQState);
	...
	private:
		...
		// < P_MBS, P_PBS_max, PBS_CellID_max,  Action(bias), Q-value > 
		//std::multiset <QState, double, double> m_qTable;
		QTABLE m_qTable;
		QState m_currentQState, m_prevQState;
		double m_lastAction;
...
private:
  ...
  void DoReportUeMeasurements (LteUeCphySapUser::UeMeasurementsParameters params);
	...
	//by Joey
	void DoSaveGlobalOutageCount (uint32_t outageCount);
  ...
  LteAsSapUser* m_asSapUser;
	...
	void StartBiasAlgorithm ();
	
	QTable* m_qTable;
	uint32_t m_globalOutageCount;
