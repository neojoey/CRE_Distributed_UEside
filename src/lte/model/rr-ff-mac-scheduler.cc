...
RrFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  // API generated by RLC for triggering the scheduling of a DL subframe
	//uint32_t tmpSubframeIdx = uint32_t(0xF & params.m_sfnSf);

  RefreshDlCqiMaps ();
  int rbgSize = GetRbgSize (m_cschedCellConfig.m_dlBandwidth);
  int rbgNum = m_cschedCellConfig.m_dlBandwidth / rbgSize;
  FfMacSchedSapUser::SchedDlConfigIndParameters ret;
	//ret.outageCount = 0;	// initializing on purpose 
  // Generate RBGs map
  ...
  rbgMap.resize (m_cschedCellConfig.m_dlBandwidth / rbgSize, false);
	...
	//mNode is a boolean used to seperate between macro and pico cell
	if (mNode == true)
	{
		cellType = "Macro cell";
	}
	...
	if (absFrame == true)
	{
		ret.absFrame = true;	//this message communicate to the MAC layer whether the subframe is protected or not.
		if (mNode == true)
		{
...
#if 0
	if ( tmpSubframeIdx % 10 == 0 || tmpSubframeIdx == 0 )
	{
		ret.outageCount = 0; 		// reset on every frame
	}
#endif
  for (it = m_rlcBufferReq.begin (); it != m_rlcBufferReq.end (); it++)
              ...
              if (itLcRnti != lcActivesPerRnti.end ())
              ...
              else
                {
                  ...
                  nTbs++;
					...
					{
						// for every UE bias algorithm by joey 2016.05.03
						ret.outageCount++;	//increase the number of outage UEs
						std::cout << Simulator::Now() << " RNTI: " << (*it).m_rnti << " ----> " << "CQI is 0........" << "Count : " << ret.outageCount << std::endl;
					}
#endif
        }
    }
#if 1
	// for Dist. bias algorithm by joey 2016.05.17
	ret.outageCount = 0; 		// reset on every subframe
	//if ( tmpSubframeIdx % 10 == 9 )
	//{
  	std::map<uint16_t,uint8_t>::iterator tmpIt;
  	for (tmpIt = m_p10CqiRxed.begin ();tmpIt != m_p10CqiRxed.end (); tmpIt++)
		{
			if ( tmpIt->second == 0 )
			{
				ret.outageCount++;
	...
	//}
	//std::cout << Simulator::Now() << " OUTAGE Count : " << ret.outageCount << std::endl;
  ...
  std::map <uint16_t,uint8_t>::iterator itTxMode;
	//for Abs by joey 2016.04.12
	bool holdFrame =false; 	//Hold this RNTI? if this is set to true the RNTI is skipped for this subframe because of bad channel conditions and ABS enabled.
  do
    {
			//for Abs
			if ((absFrame == true)&&(mNode == true))
			{
				//for check
				//std::cout << Simulator:: Now () << "  Macro enb is holding frame now.... " << std::endl;
				break;	//no RNTIs should be allocated because of subframe blanking
			}
      ...
      std::set <uint16_t>::iterator itRnti = rntiAllocated.find ((*it).m_rnti);
			...
			//for Abs by joey 2016.04.12
			std::list <uint16_t>::iterator itVictim;
			for (itVictim = victimList.begin (); itVictim != victimList.end (); itVictim++)
			{
				// ue in victim list should be served in absFrame.. so holding when it is not absFrame
				if ((*itVictim == (*it).m_rnti) && (absFrame == false))
				{
					holdFrame = true;
				}
			}

      if ((itLcRnti == lcActivesPerRnti.end ())||(itRnti != rntiAllocated.end ())||(holdFrame == true))	//holdFrame condition is added for Abs
        {
      ...
      std::map <uint16_t,uint8_t>::iterator itCqi = m_p10CqiRxed.find (newEl.m_rnti);
			...
			//for Abs by joey 2016.04.12
			//CQI ABS implementation,
			// 2 seperate CQI values based on protected/unprotected mode.
			// ------------------------------------------------------------------------------------
			uint8_t cqiTmp;
			if ((absFrame == true)&&(cqiProtected.find ((*it).m_rnti) != cqiProtected.end()))
			{
				cqiTmp = cqiProtected.find ((*it).m_rnti)->second;
			}
			else
			{
				cqiTmp = cqiNormal.find ((*it).m_rnti)->second;
			}
			//-------------------------------------------------------------------------------------
      ...
      for (uint8_t i = 0; i < nLayer; i++)
        {
          if (itCqi == m_p10CqiRxed.end ())
            ...
            {
              newDci.m_mcs.push_back ( m_amc->GetMcsFromCqi (cqiTmp) );
      ...
      uint16_t rlcPduSize = tbSize / lcNum;
			...
			//for Abs
			// Saving throughput for output calculations
			UpdateThroughputCalc ((*it).m_rnti, double (tbSize));
  ...
  ret.m_nrOfPdcchOfdmSymbols = 1;   /// \todo check correct value according the DCIs txed  
	...
	std::vector <BuildDataListElement_s>::iterator itDataEl;
	for (itDataEl = ret.m_buildDataList.begin ();itDataEl!=ret.m_buildDataList.end();itDataEl++)
	{
		std::vector <std::vector <RlcPduListElement_s> >::iterator itPduVec;
		uint32_t size = 0;
		for (itPduVec = (*itDataEl).m_rlcPduList.begin ();itPduVec != (*itDataEl).m_rlcPduList.end ();itPduVec++)
		{
			std::vector <RlcPduListElement_s>::iterator itPduEl;
			for (itPduEl = (*itPduVec).begin (); itPduEl != (*itPduVec).end (); itPduEl++)
			{
				size += (*itPduEl).m_size;
			}
		}
	...
	// 
	std::vector <ThroughputInfo>::iterator itThroughput;
	double sum = 0;
	for (itThroughput = ThroughputInfo_s.begin (); itThroughput != ThroughputInfo_s.end (); itThroughput++)
	{
		sum += (*itThroughput).lastTbSize;
		(*itThroughput).lastTbSize = 0;
	}
	tbStream.push_back (sum);
	double sum1 = 0;

	std::vector <double>::iterator itSum;
	for (itSum = tbStream.begin (); itSum != tbStream.end(); itSum++)
	{
		sum1 += *itSum;
	}
	Time timeVal = Simulator:: Now ();
	averageThroughput = sum1/double(timeVal.GetMilliSeconds ());
...
RrFfMacScheduler::DoSchedDlCqiInfoReq (const struct FfMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
	...
	if (mNode == false)
	{
		for (unsigned int iCqi = 0; iCqi < params.m_cqiList.size (); iCqi++)
		{
			if (params.m_cqiList.at (iCqi).m_cqiType == CqiListElement_s::P10 )
			{
				// for checking
				//std::cout << Simulator::Now () << " RNTI: " << int(params.m_cqiList.at (iCqi).m_rnti) << " CQI = " << int(params.m_cqiList.at (iCqi).m_wbCqi.at(0)) << std::endl;
			}
		}
	}
	uint16_t rnti;
  for (unsigned int itC = 0;itC < params.m_cqiList.size (); itC++)
  {
    rnti = params.m_cqiList.at(itC).m_rnti;
    if (params.m_cqiList.at (itC).m_cqiType == CqiListElement_s::P10)
    {
      if(params.m_cqiList.at(itC).absFrame==true)
      {
        if(cqiProtected.find(rnti)!=cqiProtected.end ())
        {
          cqiProtected.find (rnti)->second = params.m_cqiList.at(itC).m_wbCqi.at (0);
        }
        else
        {
          cqiProtected.insert (std::pair <uint16_t, uint8_t> (params.m_cqiList.at(itC).m_rnti, params.m_cqiList.at(itC).m_wbCqi.at (0)));
        }

      }
      else
      {
        if(cqiNormal.find(rnti)!=cqiNormal.end ())
        {
          cqiNormal.find (rnti)->second = params.m_cqiList.at(itC).m_wbCqi.at (0);
        }
        else
        {
          cqiNormal.insert (std::pair <uint16_t, uint8_t> (params.m_cqiList.at(itC).m_rnti, params.m_cqiList.at(itC).m_wbCqi.at (0)));
        }
      }
    }
  }
   //victim classification
	if (mNode == false)
	{
		...
		for (unsigned int i = 0; i<params.m_cqiList.size (); i++)
		{
			...
			{
				if (params.m_cqiList.at (i).m_wbCqi.at (0)<m_victimTreshold)
				{
					VictimListEdit (params.m_cqiList.at (i).m_rnti,true); //true for add, false for remove
#if 0
...
#else
					//(*picoRr.begin ())->InvokeAbs ();
					this->InvokeAbs ();
#endif
					//picoRr->InvokeAbs (); //invoke abs at the macro cell.
					victim = true;
					// for checking
					//std::cout << "Victim has been added! RNTI: " << int(params.m_cqiList.at (i).m_rnti) << " CQI = " << int(params.m_cqiList.at (i).m_wbCqi.at (0)) << std::endl;
				}
				else
				{
					VictimListEdit (params.m_cqiList.at (i).m_rnti,false);
				}
			}
		}
		if (victim == false)
...
void RrFfMacScheduler::SetAbsParameters (uint32_t p, uint16_t i)
{
	absPeriod = p;
	absInterval = i;
}
