...
#if 1
	//for Dist. algorithm by Joey 2016.05.20
	virtual void UpdateOutageCount (uint32_t outageCount);
#endif 
...
#if 1
//by Joey 2016.05.20 -> 07.14
void
EnbRrcMemberLteEnbCmacSapUser::UpdateOutageCount (uint32_t outageCount)
{
	m_rrc->DoUpdateOutageCount (outageCount);
}
#endif
  ...
  switch (m_state)
    ...
    case HANDOVER_PATH_SWITCH:
      ...
      SwitchToState (CONNECTED_NORMALLY);
			//by Joey
			SetConnectedTime (Simulator::Now ());
...
//by Joey
//for recording a connected Time and utilize it during handover procedure
void
UeManager::SetConnectedTime (Time time)
{
	m_connectedTime = time;
}

Time
UeManager::GetConnectedTime () 
{
...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
    ...
    .AddAttribute ("HandoverLeavingTimeoutDuration",
		...
		//by Joey adjust timeout duration for a test
    .AddAttribute ("HandoverJoiningTimeoutDuration",
                   "After accepting a handover request, if no RRC CONNECTION "
                   "RECONFIGURATION COMPLETE is received before this time, the "
                   "UE context is destroyed. Must account for reception of "
...
//by Joey
void
LteEnbRrc::DoRecvOutageCount (EpcX2SapUser::OutageCountParams params)
{
	NS_LOG_FUNCTION (this);

	NS_LOG_LOGIC ("Recv OutageCount through X2 infterface");
	NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId);
	NS_LOG_LOGIC ("OutageCount = " << params.outageCount);
/*
  for (std::list <uint8_t>::iterator drbIdIt = m_drbsToBeStarted.begin ();
       drbIdIt != m_drbsToBeStarted.end ();
       ++drbIdIt)
*/
	// Processes after Recv Outage Count
	if ( m_recvOutageCount.empty () ) {
		m_recvOutageCount.insert (std::pair<uint16_t, uint32_t> (params.sourceCellId, params.outageCount));
	} else {
		std::map <uint16_t, uint32_t>::iterator it = m_recvOutageCount.find (params.sourceCellId);
		if ( it != m_recvOutageCount.end () )
			it->second = params.outageCount;
		else 
			m_recvOutageCount.insert (std::pair<uint16_t, uint32_t> (params.sourceCellId, params.outageCount));
	}

#if 0
	if ( params.sourceCellId != 1 ) {
		if ( m_recvOutageCount.size() != 0 ) {
			bool tmpFlag = false; 
			for (std::list<uint8_t>::iterator it = m_recvOutageCount.begin();
					it != m_recvOutageCount.end(); ++it) {
				if ( it->first == params.sourceCellId ) {
						tmpFlag = true;
					}
			}
			if ( tmpFlag == false ) {
				m_recvOutageCount.insert ( pair<uint16_t, uint32_t> (params.sourceCellId, params.outageCount) );
			}
		} else {
				m_recvOutageFlag.push_back ( params.sourceCellId );
				m_globalOutageCount = m_globalOutageCount + params.outageCount;
		}
	}
#endif
	if ( m_recvOutageCount.size() == 6 ) {	// now # of pico BSs is 6 by force
		NS_LOG_INFO ("Outage Count are received from all Pico cells");

		// 1. sum up Pico BSs' outage count and add Macro BS's outage count to global outage count
		
		m_globalOutageCount = 0;	// reset global outage count every reception of x2 msg.
		for (std::map <uint16_t, uint32_t>::iterator it = m_recvOutageCount.begin (); it != m_recvOutageCount.end (); ++it)
		{
			m_globalOutageCount = m_globalOutageCount + it->second;
		}
		m_globalOutageCount = m_globalOutageCount + m_localOutageCount;		//add Macro BS's

		// 2. broadcast the global outage count to the pico BSs
		// but the X2 interface is p2p connection b/w eNBs
		// so, Macro cell sends it respectively to all X2 if connections at EpcX2::DoSendGlobalOutageCount ()
		EpcX2SapUser::OutageCountParams tmpParams;
		tmpParams.sourceCellId = 1;	 // 1 for Macro Cell Id
		tmpParams.outageCount = m_globalOutageCount;
		m_x2SapProvider->SendGlobalOutageCount (tmpParams);

		//reset local outage count of MBS to 0
		m_localOutageCount = 0;
		
		NS_LOG_INFO ("Macro EnbRrc:: Sending..  Global Outage Count "); 
		// 3. reset receive msg buffer.
		m_recvOutageCount.clear ();
	}
}

void
LteEnbRrc::DoRecvGlobalOutageCount (EpcX2SapUser::OutageCountParams params)
{
	NS_LOG_FUNCTION (this);

	NS_LOG_LOGIC ("Recv GlobalOutageCount through X2 infterface");
	NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId);
	...
	//ref. at code line : #2223
	m_localOutageCount = 0;

 	// save the global outage count at PHY layer to update it's bias value & Q-table
}
...
//by Joey
//////////////////////////////////////////////////////////////////
// update outage count with value recieved from mac
void
LteEnbRrc::DoUpdateOutageCount (uint32_t outageCount)
{
	//m_localOutageCount = (0.9 * m_localOutageCount) + (0.1 * outageCount);
...
uint32_t
LteEnbRrc::GetLocalOutageCount ()
{
	return m_localOutageCount;
}

uint32_t
LteEnbRrc::DoGetGlobalOutageCount ()
{
	return m_globalOutageCount;
}
///////////////////////////////////////////////////////////////
#endif
