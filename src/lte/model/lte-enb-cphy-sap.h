...
public:
  ...
  virtual ~LteEnbCphySapUser ();
	...
	//by Joey
	virtual uint32_t GetGlobalOutageCount () = 0;
...
public:
  ...
  // methods inherited from LteEnbCphySapUser go here
	virtual uint32_t GetGlobalOutageCount ();
...
template <class C>
uint32_t
MemberLteEnbCphySapUser<C>::GetGlobalOutageCount()
{
  return m_owner->DoGetGlobalOutageCount ();
}
