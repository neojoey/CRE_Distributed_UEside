...
void
QTable::InvokeBias (std::map <uint16_t, MeasValues> storedMeasValues, uint32_t outageCount, uint64_t imsi) 
{
	m_imsi = imsi;
	Quantization ( storedMeasValues );
	UpdateQTable ( m_currentQState, outageCount );
}

// Quantize the measured signal
void
QTable::Quantization ( std::map <uint16_t, MeasValues> storedMeasValues )
{
	m_quantizedMeasValues.clear ();
	std::map <uint16_t, MeasValues>::iterator it;
	for ( it = storedMeasValues.begin (); it != storedMeasValues.end (); ++it )
	{
		//MeasValue quantizedValues;
	...
	MakeState ( m_quantizedMeasValues );
}

// generates States
void
QTable::MakeState ( std::map <uint16_t, MeasValues> quantizedMeasValues )
{
	m_currentQState.pMbs = quantizedMeasValues.begin()->second.rsrp;
	double maxRsrp=-3000;
	uint16_t maxCellId;
	std::map <uint16_t, MeasValues>::iterator it;
	it = quantizedMeasValues.begin ();
	for (++it; it != quantizedMeasValues.end (); ++it ) {
...
void
QTable::UpdateQTable (QState currentQState, uint32_t outageCount)
{
	QTABLE::iterator it = m_qTable.find ( currentQState );
	if ( it != m_qTable.end () ) {
		//update Q value
		double currentMinQValue = 999999999999; 
		QTABLE::iterator prevIt = m_qTable.find ( m_prevQState );
		if ( prevIt != m_qTable.end () ) {
			//std::set <double, double> qValueIt = prevIt->second.find ( m_lastAction );
			IN_QTABLE::iterator qValueIt =  prevIt->second.find (m_lastAction);
			if ( qValueIt != prevIt->second.end () ) {
				//qValueIt->second = ((1-alpha) * qValueIt->second) + (alpha*outageCount);
				// insert loop for finding minimum q-value of the current state 2016.12.19
				for ( IN_QTABLE::iterator currIt = it->second.begin (); currIt != it->second.end (); ++currIt ) {
					if ( currIt->second < currentMinQValue ) {
						currentMinQValue = currIt->second;
					}
				}
				qValueIt->second = ((1-alpha) * qValueIt->second) + alpha*(outageCount + (gamma * currentMinQValue) ); 	// "outageCount" is a current cost
			} else {
				std::cout << "QVALUE UPDATE ERROR" << std::endl;		
			}
		}
		m_prevQState = currentQState;
	} else {
		// add a new state
		IN_QTABLE tmpInnerValue;
		//tmpInnerValue.insert (std::make_pair( m_lastAction, outageCount ));
...
void
QTable::SelectAction (QState currentQState)
{
	double actionBias;
	QTABLE::iterator it = m_qTable.find ( currentQState );
	...
	if ( it->second.size () < 16 ) {
		...
		actionBias = 0;
		/*
		actionBias = it->second.size ();	//there is basic element, bias = 0 
		// insert new bias table
		it->second.insert( std::make_pair( actionBias, 0 ) );
		*/
	} else {
		...
		std::map<double, double> sameValue;
		for ( actionIt = it->second.begin (); actionIt != it->second.end (); ++actionIt ) {
			if ( (*actionIt).second == minQValue ) {
				// ( bias, Q-value )
				sameValue.insert (std::make_pair ( actionIt->first, actionIt->second) );
			}
		}
...
/*
void
QTable::ExecuteAction (double bias, uint16_t cellId) {
	m_cphySapProvider->UpdateBias (bias, cellId);
}
*/
uint16_t 
QTable::GetMaxCellId ()
{
	return m_currentQState.picoCellId;
}
...
LteUeRrc::DoInitialize (void)
{
  ...
  m_cmacSapProvider->AddLc (lcid, lcConfig, rlc->GetLteMacSapUser ());
	...
	Simulator::Schedule ( MilliSeconds(300), &LteUeRrc::StartBiasAlgorithm, this);
}
...
//by Joey
void
LteUeRrc::DoSaveGlobalOutageCount (uint32_t outageCount)
{
	m_globalOutageCount = outageCount;
	//InvokeBias (m_sotredMeasValues, outageCount);
#if 0
	std::map<uint16_t, QTable::MeasValue> tmpValue;
	for ( std::map<uint16_t, MeasValues>::iterator it = m_storedMeasValues.begin ();
			it != m_storedMeasValues.end ();
			++it ) 
	{
		tmpValue.insert (std::make_pair ((*it).first, (*it).second));
	}
#endif
	//Simulator::Schedule ( MilliSeconds(500), &QTable::InvokeBias, m_qTable, m_storedMeasValues, outageCount );
...
//by Joey
void
LteUeRrc::CheckOutageCount ()
{
  uint16_t cellId = m_lastSib1.cellAccessRelatedInfo.cellIdentity;
	uint32_t outageCount = m_lastSib1.outageCount;
	std::cout << "IMSI : " << m_imsi << " UE recieves outage count : " << outageCount << " from CID : " << cellId << std::endl;	
}
#endif
