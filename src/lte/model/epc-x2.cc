...
EpcX2::RecvFromX2cSocket (Ptr<Socket> socket)
{
  ...
  if (procedureCode == EpcX2Header::HandoverPreparation)
	...
	//by Joey
	else if (procedureCode == EpcX2Header::OutageCount)
    {
      if (messageType == EpcX2Header::InitiatingMessage)
        {
          NS_LOG_LOGIC ("Recv X2 message: Outage Count Information");

          EpcX2OutageCountHeader x2OutageCountHeader;
          packet->RemoveHeader (x2OutageCountHeader);

          NS_LOG_INFO ("X2 OutageCount header: " << x2OutageCountHeader);

          EpcX2SapUser::OutageCountParams params;
          params.sourceCellId = x2OutageCountHeader.GetSourceCellId ();
          params.outageCount= x2OutageCountHeader.GetOutageCount ();

          m_x2SapUser->RecvOutageCount (params);
        }
    }
	else if (procedureCode == EpcX2Header::GlobalOutageCount)
    {
      if (messageType == EpcX2Header::InitiatingMessage)
        {
          NS_LOG_LOGIC ("Recv X2 message: Outage Count Information");

          EpcX2GlobalOutageCountHeader x2GlobalOutageCountHeader;
          packet->RemoveHeader (x2GlobalOutageCountHeader);

          NS_LOG_INFO ("X2 OutageCount header: " << x2GlobalOutageCountHeader);

          EpcX2SapUser::OutageCountParams params;
          params.sourceCellId = x2GlobalOutageCountHeader.GetSourceCellId ();
          params.outageCount= x2GlobalOutageCountHeader.GetOutageCount ();

          m_x2SapUser->RecvGlobalOutageCount (params);
        }
    }
...
//by Joey
//Pico BSs send a local outage count to the Macro BS
void 
EpcX2::DoSendOutageCount (EpcX2SapProvider::OutageCountParams params)
{
  NS_LOG_FUNCTION (this);
  Ptr<X2IfaceInfo> socketInfo = m_x2InterfaceSockets [1]; 	// to macro cell whose id is 1
  Ptr<Socket> sourceSocket = socketInfo->m_localCtrlPlaneSocket;
  //Ptr<Socket> sourceSocket = socketInfo->m_localUserPlaneSocket;
  Ipv4Address targetIpAddr = socketInfo->m_remoteIpAddr;
 
	NS_LOG_INFO ("Source ID: " << params.sourceCellId << " to Addr : " << targetIpAddr);
	// Build the X2 message
  EpcX2OutageCountHeader x2OutageCountHeader;
  x2OutageCountHeader.SetOutageCount (params);

  EpcX2Header x2Header;
  x2Header.SetMessageType (EpcX2Header::InitiatingMessage);
  x2Header.SetProcedureCode (EpcX2Header::OutageCount);
  x2Header.SetLengthOfIes (x2OutageCountHeader.GetLengthOfIes ());
  x2Header.SetNumberOfIes (x2OutageCountHeader.GetNumberOfIes ());

  NS_LOG_INFO ("X2 header: " << x2Header);
  NS_LOG_INFO ("X2 Outage Count  header: " << x2OutageCountHeader);

  // Build the X2 packet
  Ptr<Packet> packet = Create <Packet> ();
  packet->AddHeader (x2OutageCountHeader);
  packet->AddHeader (x2Header);
  NS_LOG_INFO ("packetLen = " << packet->GetSize ());
  ...
  sourceSocket->SendTo (packet, 0, InetSocketAddress (targetIpAddr, m_x2cUdpPort));
	...
	NS_LOG_INFO ("Send outage count from " << params.sourceCellId);
}

// Macro BS send a global outage count to the Pico BSs
void 
EpcX2::DoSendGlobalOutageCount (EpcX2SapProvider::OutageCountParams params)
{
  NS_LOG_FUNCTION (this);

  NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId);

	for ( std::map < uint16_t, Ptr<X2IfaceInfo> >::iterator it = m_x2InterfaceSockets.begin (); it != m_x2InterfaceSockets.end (); ++it)
	{
		Ptr<X2IfaceInfo> socketInfo = it->second; 	// to macro cell whose id is 1
  	Ptr<Socket> sourceSocket = socketInfo->m_localCtrlPlaneSocket;
  	Ipv4Address targetIpAddr = socketInfo->m_remoteIpAddr;
  	...
  	EpcX2Header x2Header;
  	x2Header.SetMessageType (EpcX2Header::InitiatingMessage);
  	x2Header.SetProcedureCode (EpcX2Header::GlobalOutageCount);
  	x2Header.SetLengthOfIes (x2OutageCountHeader.GetLengthOfIes ());
  	x2Header.SetNumberOfIes (x2OutageCountHeader.GetNumberOfIes ());
  	...
  	// Send the X2 message through the socket
  	sourceSocket->SendTo (packet, 0, InetSocketAddress (targetIpAddr, m_x2cUdpPort));
		NS_LOG_INFO ("Send Global outage count from " << params.sourceCellId << " to " << targetIpAddr);
	}
}
