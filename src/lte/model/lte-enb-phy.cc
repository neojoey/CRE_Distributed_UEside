...
//by Joey
void
EnbMemberLteEnbPhySapProvider::UpdateOutageCount (uint32_t outageCount)
{
	m_phy->DoUpdateOutageCount (outageCount);
}
...
LteEnbPhy::LteEnbPhy (Ptr<LteSpectrumPhy> dlPhy, Ptr<LteSpectrumPhy> ulPhy)
  : LtePhy (dlPhy, ulPhy),
    ...
    m_nrFrames (0),
    m_nrSubFrames (0),
		absFrame (false), 	// for Abs by joey 2016.04.12
    m_srsPeriodicity (0),
...
//by Joey
void
LteEnbPhy::DoUpdateOutageCount (uint32_t outageCount)
{
	// average 10 subframe
	std::cout << "Before : " << m_outageCount << "New value : " << outageCount<< std::endl;
	m_outageCount = double(0.9 * m_outageCount) + double(0.1 * outageCount);
	std::cout << "After : " << m_outageCount << std::endl;
}
...
LteEnbPhy::StartSubFrame (void)
{
  ...
  if ((m_nrSubFrames == 6) && ((m_nrFrames % 2) == 1))
    {
      ...
      m_controlMessagesQueue.at (0).push_back (msg);
			...
			//by Joey
			//Send outage count with same cycle of SIB1 control message
			Ptr<OutageCtrlMessage> outageMsg = Create<OutageCtrlMessage> ();
			m_globalOutageCount = m_enbCphySapUser->GetGlobalOutageCount ();
			outageMsg->SetOutageCount (m_globalOutageCount);
			m_controlMessagesQueue.at (0).push_back (outageMsg);
			//std::cout << Simulator::Now() << " Enb Sends Outage Count : " << m_globalOutageCount << std::endl;
			m_globalOutageCount = 0;	// reset after sending
    }
  ...
  m_dlPowerAllocationMap.clear ();
	bool absFrame = false;		//for Abs by joey 2016.04.12
  if (ctrlMsg.size () > 0)
    {
      std::list<Ptr<LteControlMessage> >::iterator it;
      it = ctrlMsg.begin ();
      while (it != ctrlMsg.end ())
        {
          Ptr<LteControlMessage> msg = (*it);
					...
					if (msg->GetMessageType () == LteControlMessage::ABS)
					{
						Ptr<AbsCtrlMessage> abs = DynamicCast<AbsCtrlMessage> (msg);
						absFrame = abs->GetAbs ();
					}
	...
	{
		SetTxPower (23);	//lowering txpower as a result of subframe blanking
		//std::cout << "abs is operating now....Q!" << std::endl;
	}
	//--------------
...
LteEnbPhy::SendControlChannels (std::list<Ptr<LteControlMessage> > ctrlMsgList)
{
  ...
  if ((m_nrSubFrames == 1) || (m_nrSubFrames == 6))
	...
	if (absFrame == true)
	{
		//std::cout << Simulator::Now() << "ABS phy" << std::endl;
	}
	//-------------
