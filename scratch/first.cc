...
#if 1
void PlotThroughputCdf ( double data_array1[], double data_array2[], int num, uint16_t absInterval, double simTime )
{
	...
	std::string fileNameWithNoExtension = "1st_throughput-cdf_i"+condAbs+"_t"+condTime;
...
void PlotDelay ( double data_array1[], int num, uint16_t absInterval, double simTime)
{
	std::stringstream ss1;
	ss1 << absInterval;
	std::string condAbs = ss1.str();
	...
	std::string fileNameWithNoExtension = "1st_delay_i"+condAbs+"_t"+condTime;
	std::string graphicsFileName = fileNameWithNoExtension + ".png";
	std::string plotFileName = fileNameWithNoExtension + ".plt";
...
void PlotHOCdf( double data_array1[], double data_array2[], int num, uint16_t absInterval, double simTime )
{
	std::stringstream ss1;
	ss1 << absInterval;
	std::string condAbs = ss1.str();
	...
	std::string fileNameWithNoExtension = "1st_handoverCount-cdf_i"+condAbs+"_t"+condTime;
	std::string plotFileName = fileNameWithNoExtension + ".plt";
	std::string graphicsFileName = fileNameWithNoExtension + ".png";
...
#ifdef ANIMATION
	std::string animFile = "SimpleAnim.xml";
#endif
...
//	std::cout << "nUes= " << nUes << ", offeredLoad= " << offeredLoad << ", nPicoEnb= " << nPicoEnb << ", scenariotype = " << scenarioType << std::endl;
	...
	//lteHelper->SetHandoverAlgorithmAttribute ("TimeToTrigger", TimeValue (MilliSeconds (500)));	//  during 50ms
...
//
	...
	//Flow monitor
	monitor->SerializeToXmlFile("flowen.flow", true, true);
	...
	PlotThroughputCdf (throughputArray, percentageThroughput , nUes, absInterval, simTime);
	PlotDelay (delayArray, nUes, absInterval, simTime);
	PlotHOCdf(hoArray, percentageHO, nUes, absInterval, simTime);
